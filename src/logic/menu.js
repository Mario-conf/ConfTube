...
export class Funciones {
    leftmenu() {
        ...
        if (sidebar) {
            console.log("Elemento 'sideBar' encontrado en el DOM.");

            // Limpiar el contenido existente del contenedor
            sidebar.innerHTML = '';

            // Agregar dinámicamente los botones al contenido existente
            sidebar.insertAdjacentHTML('beforeend', `
                <!-- main home explore btns -->
                <div class="flex flex-col w-full mb-2">
                    ${this.leftmenuButton('home', 'Home', true)}
                    ${this.leftmenuButton('explore', 'Explore')}
                    ${this.leftmenuButton('subscriptions', 'Subscriptions')}
                </div>
                <div class="border border-searchIconBg"></div>
                <!-- library history btns -->
                <div class="flex flex-col w-full mb-2 mt-3">
                    ${this.leftmenuButton('video_library', 'Library')}
                    ${this.leftmenuButton('history', 'History')}
                    ${this.leftmenuButton('slideshow', 'Your videos')}
                    ${this.leftmenuButton('watch_later', 'Watch later')}
                    ${this.leftmenuButton('thumb_up', 'Liked videos')}
                </div>
            `);
        } else {
            console.warn("Elemento 'sideBar' no encontrado en el DOM.");
        }
    }

    leftmenuButton(icon, label, active = false) {
    const activeClass = active ? 'bg-sideBarHoverBg text-white' : 'text-gray-400';
    return `
        <div class="flex items-center space-x-5 hover:bg-sideBarHoverBg px-7 py-2.5 cursor-pointer ${activeClass}">
            <i class="material-icons md-21">${icon}</i>
            <h2 class="text-sm ${active ? 'font-semibold text-white' : 'text-gray-400'}">${label}</h2>
        </div>
    `;
}
    ...
    const navHtml = `
        <div class="flex justify-between items-center px-2 sm:px-6 py-1.5 mx-auto">
            ...
            <div class="flex items-center space-x-3 sm:space-x-6 pr-4">
                ...
                <img class="w-9 h-9 rounded-full object-cover cursor-pointer" src="https://avatars.githubusercontent.com/u/152533010?v=4" alt="" draggable="false">
            </div>
        </div>
    `;
    const nav = document.querySelector('nav.bg-topNavBg');
    if (nav) {
        nav.innerHTML = navHtml;
    } else {
        console.warn("Navbar element with selector 'nav.bg-topNavBg' not found in the DOM.");
    }
}

    uppermenu() {
    const tagList = [
        { tagLabel: "All", active: true },
        { tagLabel: "CSS" }, { tagLabel: "HTML" }, { tagLabel: "Java" },
        { tagLabel: "AWS" }, { tagLabel: "Linux" }, { tagLabel: "MAV" },
        { tagLabel: "YouTube" }
    ];

    const tagsContainer = document.querySelector('.tags-container');
    if (tagsContainer) {
        console.log("Elemento 'tags-container' encontrado en el DOM.");

        // Limpiar el contenido existente del contenedor
        tagsContainer.innerHTML = '';

        // Agregar dinámicamente las etiquetas al contenido existente
        tagList.forEach(tag => {
            tagsContainer.insertAdjacentHTML('beforeend', this.uppermenuTag(tag.tagLabel, tag.active));
        });
    } else {
        console.warn("Tags container con clase 'tags-container' no encontrado en el DOM.");
    }
    }

    uppermenuTag(tagLabel, active = false) {
        if (active) {
            return `<a href="#" class="text-sm bg-white py-1.5 px-3 rounded-full text-black">${tagLabel}</a>`;
        }
        return `<a href="#" class="text-sm bg-pillsBg hover:bg-gray-700 py-1 px-3 rounded-full text-white border border-gray-700 transition duration-150">${tagLabel}</a>`;
    }
                ...
                <div class="flex flex-col space-y-3 cursor-pointer">
                    <a href="#">
                        <div class="relative h-52 sm:h-36 w-full">
                            <img class="relative h-full w-full object-cover" src="" alt="">
                            <span class="absolute right-1 bottom-1 p-0.5 px-1 rounded-sm bg-black text-white text-xs bg-opacity-70">12:34</span>
                        </div>
                    </a>
                    <div class="flex flex-col flex-auto space-y-1.5 mb-3">
                        <div class="flex items-center space-x-3">
                            <img class="w-12 h-12 rounded-full object-cover cursor-pointer" src="https://avatars.githubusercontent.com/u/152533010?v=4" alt="">
                            <div class="flex items-start">
                                <a href="#" class="text-sm text-bold text-white pr-1">Nuevo video dinámico</a>
                                <i class="material-icons text-white md-21">more_vert</i>
                            </div>
                        </div>
                        <div class="flex flex-col pl-1 sm:pl-0 ml-14">
                            <a href="" class="text-gray-400 text-sm hover:text-white">Mario.Conf</a>
                            <div class="flex space-x-1">
                                <span class="text-sm text-gray-400">10K views</span>
                                <span class="text-sm text-gray-400">• 2 días atrás</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    addElement(parentSelector, htmlContent) {
        const parent = document.querySelector(parentSelector);
        if (parent) {
            parent.innerHTML += htmlContent;
        } else {
            console.warn(`El elemento ${parentSelector} no se encontró en el DOM.`);
        }
    }
    ...
    initMenus() {
        try {
            ...
            console.error('Error al inicializar el menú superior:', error);
        }
        this.leftmenu();
        if (user.isLoggedIn) {
            this.navbar();
